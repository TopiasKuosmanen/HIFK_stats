...
GO
PRINT N'Creating [dbo].[Player_FilterByLastName]...';


GO
CREATE PROCEDURE [dbo].[Player_FilterByLastName]
	@PlayerName NVARCHAR (50)
AS
BEGIN
	SELECT Id, FirstName, LastName, BirthDate, FLOOR((CAST (GETDATE() AS INTEGER) - CAST(BirthDate AS INTEGER)) / 365.25) AS Age
	FROM dbo.Player
	WHERE LastName LIKE '%' + @PlayerName + '%' OR FirstName LIKE '%' + @PlayerName + '%'
	ORDER BY LastName
END
GO
